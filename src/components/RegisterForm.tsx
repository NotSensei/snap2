'use client'
import { ArrowLeft, EyeIcon, EyeOff, KeyRound, Leaf, Loader2, LogIn, Mail, User } from 'lucide-react'
...
function RegisterForm({ prevStep }: propType) {
    ...
    return (
        <div className='flex flex-col items-center justify-center min-h-screen px-6 py-10 bg-white relative'>
            <div className='absolute top-6 left-6 flex items-center gap-2 text-green-700 hover:text-green-800 transition-colors cursor-pointer' onClick={() => prevStep(1)}>
            ...
            <p className='text-gray-600 mb-8 flex items-center'>Join SnapCart today <Leaf className='w-5 h-5 text-green-600 ml-1' /></p>
            <motion.form
                ...
                <p className='text-gray-600 mt-6 text-sm flex items-center gap-1 cursor-pointer' onClick={() => router.push('/login')}>Already have an account ? <LogIn className='w-4 h-4' /> <span className='text-green-500'> Sign in</span></p>
        </div>
    )
}
