...
import { Box, ClipboardCheck, Cross, LogOut, Menu, Package, PlusCircle, Search, ShoppingCartIcon, User, X } from 'lucide-react'
        ...
        <AnimatePresence>
            ...
            >
                <div className='flex justify-between items-center mb-2'>
                    ...
                    <button className='text-whhite/80 hover:text-red-400 text-2xl font-bold transition' onClick={() => setMenuOpen(false)}><X /></button>
                </div>
                ...
                <div className='flex flex-col gap-3 font-medium mt-6'>
                    ...
                    <Link href={"/admin/view-grocery"} className='flex items-center gap-3 p-3 rounded-lg bg-white/10 hover:bg-white/20 hover:pl-4 transition-all'><Box className='w-5 h-5' /> View Grocery</Link>
    ...
    return (
        <div className='w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50'>
            ...
            {user.role == "user" && <form className='hidden md:flex items-center bg-white rounded-full px-4 py-2 w-1/2 max-w-lg shadow-md' onSubmit={handleSearch}>
                ...
                <input className='outline-none' type="text" placeholder='Search groceries...' value={search} onChange={(e) => setSearch(e.target.value)}/>
            </form>}
            ...
            <div className='flex items-center gap-3 md:gap-6 relative'>
                ...
                {user.role == "admin" && <>
                    <div className='hidden md:flex items-center gap-4'>
                        ...
                        <Link href={"/admin/view-grocery"} className='flex items-center gap-2 bg-white text-green-700 font-semibold px-4 py-2 rounded-full hover:bg-green-100 transition-all'><Box className='w-5 h-5' /> View Grocery</Link>
                    ...
                    <AnimatePresence>
                        {open &&
                            ...
                            >
                                ...
                                <button className='flex items-center gap-3 w-full text-left px-3 py-3 hover:bg-red-50 rounded-lg text-gray-700 font-medium' onClick={() => {
                                    setOpen(false)
