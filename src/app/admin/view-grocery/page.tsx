...
import { ArrowLeft, CloudUpload, Loader, Package, Pencil, Search, Upload, X } from 'lucide-react'
import { useRouter } from 'next/navigation'
import { IGrocery } from '@/models/grocey.model'
                    ...
                    >
                        ...
                        <div className='flex-1 flex flex-col justify-between w-full'>
                            ...
                            <div className='mt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2'>
                                ...
                                <button className='bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-2 hover:bg-green-700 transition-all cursor-pointer' onClick={() => setEditing(g)}>
                                    <Pencil size={15} /> Edit
                                </button>
                            </div>
                        </div>
                    ...
                    >
                        ...
                        >
                            <div className='flex justify-between items-center mb-4'>
                                ...
                                <button className='text-gray-600 hover:text-red-600 cursor-pointer' onClick={() => setEditing(null)}>
                                    <X size={18} />
                                </button>
                            </div>
                            <div className='relative aspect-square w-full rounded-lg overflow-hidden mb-4 border border-gray-200 group'>
                                {imagePreview &&
                                    ...
                                    />}
                                    <label htmlFor='imageUpload' className='absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center cursor-pointer transition-opacity'><CloudUpload size={28} className='text-green-500'/></label>
                            ...
                            <div className='flex justify-end gap-3 mt-6'>
                                <button className='px-4 py-2 rounded-lg bg-green-600 text-white flex items-center gap-2 hover:bg-green-700 transition-all cursor-pointer' onClick={handleEdit} disabled={editLoading}>
                                    {editLoading ? <Loader size={14} className='animate-spin' /> : "Edit Grocery"}
                                </button>
                                <button className='px-4 py-2 rounded-lg bg-red-600 text-white flex items-center gap-2 hover:bg-red-700 transition cursor-pointer' onClick={handleDelete} disabled={deleteLoading}>
